...
int mandelbrot_draw(int x_resolution, int y_resolution, int max_iter,
                    ...
                    double power, bool no_output, int tid) {
    ...
    int i = tid;
    for (i; i < y_resolution; i+=NUM_THREADS) {
...
void* thread_function(void *arguments){
    struct thread_args *args = static_cast<thread_args *>(arguments);
    int tid = args->tid;
    /**
     * (int x_resolution, int y_resolution, int max_iter,
                    double view_x0, double view_x1, double view_y0, double view_y1,
                    double x_stepsize, double y_stepsize,
                    int palette_shift, unsigned char *img,
                    double power, bool no_output)
     */
    int numSamplesInSet = mandelbrot_draw(
            args->x_resolution,
            args->y_resolution,
            args->max_iter,
            args->view_x0,args->view_x1,
            args->view_y0,args->view_y1,
            args->x_stepsize,args->y_stepsize,
            args->palette_shift, args->img, args->power, args->no_output,
    args->tid);
    args->pointsInSetCount = numSamplesInSet;
...
int main(int argc, char **argv) {
    ...
    // Initialize argument structure passed to the threads

    pthread_t threads[NUM_THREADS];

    struct thread_args base_args;
    base_args.x_resolution = x_resolution;
    base_args.y_resolution = y_resolution;
    base_args.max_iter = max_iter;
    base_args.view_x0 = view_x0;
    base_args.view_x1 = view_x1;
    base_args.view_y0 = view_y0;
    base_args.view_y1 = view_y1;
    base_args.x_stepsize = x_stepsize;
    base_args.y_stepsize = y_stepsize;
    base_args.palette_shift = palette_shift;
    base_args.img = image;
    base_args.power = power;
    base_args.no_output = no_output;

    struct thread_args custom_args[NUM_THREADS];
    // Initialize threads, pass every thread an ID (0 - NUM_THREADS)
    for (int i = 0 ; i < NUM_THREADS ; i++){
        custom_args[i] = base_args;
        custom_args[i].tid = i;
        pthread_create(&threads[i], NULL, &thread_function, &custom_args[i]);
    }
    ...
    */
    for (int i = 0 ; i < NUM_THREADS ; i++){
        ...
        numSamplesInSet += custom_args[i].pointsInSetCount;
    }
